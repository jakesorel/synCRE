...
pyGenomeTracks --tracks %s --region %s:%d-%d -o %s --width %.2f --fontSize 6 >/dev/null 2>&1
    ...
    def write_atac(self,f,source_file=None,colors=None,min_value = 0,max_value=20):
        ...
        for bwname, bwdir in source_file.values:
            if "#" not in bwname:
                f.write(self.make_bigwig(bwname, bwdir,color='#%02x%02x%02x' % tuple((colors[k][:3]*256).astype(np.int64)),min_value=min_value,max_value=max_value))
                k+=1
