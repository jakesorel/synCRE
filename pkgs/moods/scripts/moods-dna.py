...
option_group.add_argument('--threshold-precision', metavar='x', action='store', dest='threshold_precision', type=float, help='specify the precision used for computing the thresholds from p-values (default = {})'.format(
    MOODS.tools.DEFAULT_DP_PRECISION), default = MOODS.tools.DEFAULT_DP_PRECISION)
    ...
    for seq_file in args.sequence_files:
        ...
        for header, seq in seq_iterator:
            ...
            if args.p_val is not None and not args.batch:
                ...
                thresholds = [
                    MOODS.tools.threshold_from_p_with_precision(m, bg, args.p_val, args.threshold_precision, 4) for m in matrices_all]
                if args.verbosity >= 3:
                    for (m, t) in zip(matrix_names, thresholds):
                        print("{}: threshold for {} is {}".format(os.path.basename(__file__), m, t), file=sys.stderr)
                if args.verbosity >= 1:
